...
| # | 位置 | 问题 | 修复方案 | 状态 |
|---|------|------|----------|------|
| 1 | `action.yml:42` | **GitHub Action 命令注入**: `${{ inputs.diff-ref }}` 直接拼入 shell，恶意 PR 可注入任意命令 | 改用 `env:` 传递 inputs，shell 中用 `"$VAR"` 引用 | ✅ 已修复 |
| 2 | `src/cli/commands/scan.ts:27` | `parseInt(--min-score)` 无 NaN 校验，传 `abc` 时 `score < NaN` 始终 false，**CI 永不失败** | 加 `isNaN` 校验，无效值时报错退出 | ✅ 已修复 |
| 3 | `src/core/engine.ts:376-403` | **Fingerprint 包含绝对行号**，文件头部插入一行后所有 fingerprint 变化，baseline 差异全部显示为 "new" | 改用代码内容哈希替代 `startLine:endLine`（Phase 3 已记录实现，但未标注此缺陷） | ✅ 已修复 |
| 4 | 多个规则: `empty-catch`, `dead-logic`, `unnecessary-try-catch` | **手动花括号计数提取代码块**，字符串/模板/正则中的 `{`/`}` 导致边界错误 → 误报/漏报 | 新增 `brace-utils.ts` 字符串/注释感知计数 | ✅ 已修复 |

#### P1 — 规则检测质量（直接影响扫描可信度）

| # | 位置 | 问题 | 修复方案 | 状态 |
|---|------|------|----------|------|
| 5 | `missing-await.ts`, `promise-void.ts` | 只检测**本文件内定义的** async 函数，无法检测 imported 异步函数（如 `fs.promises.readFile`） | 需要跨文件符号表或 import 追踪机制 | 待定 |
| 6 | `unused-variables.ts:140` | 未使用变量检测遗漏 class 名、方法名、enum 成员、解构属性等声明形式 → 误报 | 扩展排除的 AST parentType 列表 | ✅ 已修复 |
| 7 | `duplicate-condition.ts:86` | 未处理的 AST 节点类型都返回 `[node.type]`，两个不同复杂表达式被误判为重复条件 | 为更多节点类型实现 stringify，或对未知类型加位置区分 | ✅ 已修复 |
| 8 | `unused-import.ts:130` | 用 `/\b[A-Z][A-Za-z0-9]*\b/g` 在注释中查找引用，注释中碰巧出现同名标识符导致**漏报** | 只在代码行（非注释/字符串）中检查引用 | ✅ 已修复 |
| 9 | `src/core/scorer.ts:7-12` | 评分用**固定扣分值**，不按代码量归一化，7 个 high 直接扣到 0 分，大文件不公平 | 改为 diminishing penalty（0.7 衰减因子） | ✅ 已修复 |
| 10 | `src/core/scorer.ts:31-36` | 维度 key 硬编码，新增 `RuleCategory` 需手动同步 | 从 RuleCategory enum 自动派生 | 待定 |
| 11 | `src/core/config.ts` vs `src/types/config.ts` | `generateDefaultConfig` 缺少 `max-cognitive-complexity` 字段，用户生成的配置不完整 | 补齐字段 | ✅ 已修复 |

#### P2 — 性能问题

| # | 位置 | 问题 | 修复方案 | 状态 |
|---|------|------|----------|------|
| 12 | `src/rules/fix-utils.ts:8,21` | `lineStartOffset`/`lineRange` 每次调用都 `content.split('\n')`，大文件多 issue = O(N*M) | 预计算行偏移表 `buildLineOffsets` | ✅ 已修复 |
| 13 | `src/analyzers/baseline.ts:82` | `collectFiles` 用 `readdirSync` 递归遍历，monorepo 阻塞主线程 | 改用 `fs.promises` 异步遍历 | 待定 |
| 14 | `src/core/engine.ts:539-561` | `groupByDimension` 对 issues 做 5 次 filter 遍历 | 改为单次 for 循环 | ✅ 已修复 |
| 15 | `src/core/fix-engine.ts:79` | 指定 `--rule` 时仍执行所有规则再过滤 | 传 ruleId filter 到 RuleEngine.run | 待定 |
| 16 | `src/core/fix-engine.ts` | `fix` 方法是 async 但内部用 `readFileSync`/`writeFileSync` | 统一为 `fs.promises` | 待定 |
...
| # | 位置 | 问题 | 修复方案 | 状态 |
|---|------|------|----------|------|
| 17 | `missing-await.ts`, `promise-void.ts` | async 函数收集逻辑重复 | 提取共享 `collectAsyncFunctions` 工具函数 | 待定 |
| 18 | ~10 个规则文件 | 各自实现 `inBlockComment` 注释跟踪 | 抽成 `src/rules/utils/comment-tracker.ts` | 待定 |
| 19 | `src/core/engine.ts:362-370` | 自制 glob→regex 转换，边界 case 多 | 用 `micromatch` 或 `picomatch` 替代 | 待定 |
| 20 | `src/parsers/ast.ts:101` | 箭头函数统一命名 `<arrow>`，丢失 `const myFunc = () => {}` 的名称 | 检查父节点 VariableDeclarator 获取名称 | ✅ 已修复 |
| 21 | `src/parsers/diff.ts:110` | Diff 路径解析正则无法处理文件名含空格或 ` b/` 的情况 | 优先用 `+++ b/` 行提取路径 | ✅ 已修复 |
| 22 | `src/types/index.ts:114-121` | `DiffFile.status` 缺少 `copied` / `type changed` | 补齐 Git diff status 类型 | 待定 |
| 23 | `src/cli/commands/hook.ts:40-44` | pre-commit hook 已存在时直接跳过，不提供追加选项 | 提供 `--force` 覆盖或追加模式 | 待定 |
| 24 | `src/i18n/index.ts:38-51` | macOS locale 检测执行同步 shell 命令阻塞事件循环 | 改为 `execFileSync` 或缓存后异步 | 待定 |
